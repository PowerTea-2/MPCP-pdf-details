 ...
 *   gcc -O2 AethroSync_2_.c -o mpcp -lsodium -lzstd -lm -lpthread
 *
 * Run:
 ...
 *   ./mpcp --debug-stealth - enable Blink missed-window diagnostics
 *
...
/* Blink Engine: --debug-stealth flag prints missed receive windows */
static bool g_debug_stealth = false;
/* Blink Engine: global missed-window counter (atomic) */
static _Atomic(uint64_t) g_blink_missed_windows = 0;
/* Blink Engine: global on-time counter (atomic) */
...
/* Chunk overhead: seq(4)+retry(1)+flags(1)+plen(4)+nonce(24)+sched_time(8)+tag(16) */
#define MPCP_CHUNK_OVERHEAD      (4+1+1+4+24+8+16)
#define MPCP_DATA_HDR_OFFSET     48u   /* offset where ciphertext starts (includes 8B sched_time) */
...
- ----------------------------------- */
  ...
  uint32_t    port_hop_interval;  /* Blink: hop every N chunks, 0=per-chunk */
  uint32_t    blink_interval_us;  /* Blink: inter-chunk transmit interval (µs), 0=auto */
...
- ----------------------------------- */
  ...
  /* Blink Engine clock synchronisation (set during calibration) */
  int64_t  clock_offset_ns;  /* receiver_clock - sender_clock delta (ns) */
  uint64_t blink_interval_ns;/* inter-chunk transmission interval (ns) */
  uint64_t blink_origin_ns;  /* sender absolute origin time for sequence 0 */
  } mpcp_session_t;
 ...
 *
 * Layout: hdr(6) + seq(2) + send_ts(8) + nonce_hint(32) + padding(16) = 64
 * Blink Engine: send_ts carries CLOCK_MONOTONIC nanoseconds from sender.
 * Receiver computes clock_offset = (recv_ts - send_ts) - RTT/2 after cal.
 */
  typedef struct __attribute__((packed)) {
  mpcp_hdr_t hdr;
  uint16_t   seq;
  uint64_t   send_ts;            /* CLOCK_MONOTONIC ns (sender) — Blink sync */
...
/* Data/ghost chunk header (first MPCP_DATA_HDR_OFFSET bytes, S18.3 + Blink)
 * Layout: hdr(6)+seq(4)+retry(1)+flags(1)+plen(4)+nonce(24)+sched_time(8) = 48 */
typedef struct __attribute__((packed)) {
mpcp_hdr_t hdr;
uint32_t   seq_index;
uint8_t    retry_count;
...
uint8_t    xchacha_nonce[24];
uint64_t   scheduled_time_ns; /* Blink: sender CLOCK_MONOTONIC transmit time (BE) */
...
/* =========================================================
 * Blink Engine — Temporal Stealth helpers
 *
 * Clock sync:  clock_offset_ns = avg(recv_ts_i - send_ts_i - rtt_i/2)
 * T_arrival_i = blink_origin_ns + i*blink_interval_ns
 *               - clock_offset_ns          (convert to receiver clock)
 *               + rtt_ns/2                 (one-way flight estimate)
 * Guard band:  2 ms pre-sleep, 5 ms poll window
 *
 * Port hopping: every cfg->port_hop_interval chunks, derive new port
 *   hop_port(session_key, session_nonce, chunk_idx, hop_interval,
 *            port_base, port_range)
 *   Uses HKDF(session_key, "blink-hop" || chunk_idx_base || nonce)
 * ========================================================= */

/* Derive the Blink hop port for chunk chunk_idx.
 * Both sender and receiver call this with identical arguments. */
static uint16_t blink_hop_port(const mpcp_session_t *sess,
                                const mpcp_config_t  *cfg,
                                uint32_t              chunk_idx);

/* Compute receiver-local expected arrival time (CLOCK_MONOTONIC ns)
 * for chunk chunk_idx, given calibrated session parameters. */
static uint64_t blink_arrival_time(const mpcp_session_t *sess,
                                    uint32_t              chunk_idx);

/* Sleep until (target_ns - lead_ns), then poll fd for at most
 * guard_ns nanoseconds. Returns bytes received (>=0) or -1 on timeout.
 * If data arrives, it is placed into buf (up to buf_len bytes). */
static ssize_t blink_recv_window(int fd, void *buf, size_t buf_len,
                                  uint64_t target_ns, uint64_t guard_ns,
                                  struct sockaddr_in *from_addr,
                                  socklen_t *from_len);

/* Token-bucket pacer state for sender */
typedef struct {
    uint64_t next_send_ns;   /* absolute CLOCK_MONOTONIC time of next send */
    uint64_t interval_ns;    /* inter-chunk interval (nanoseconds) */
} blink_pacer_t;

static void blink_pacer_init(blink_pacer_t *p, uint64_t interval_ns,
                              uint64_t origin_ns);
static void blink_pacer_wait(blink_pacer_t *p); /* sleep until next slot */

/* Fixed-size sliding window for missed/hit accounting (no malloc) */
#define BLINK_WINDOW_SIZE 128u
typedef struct {
    uint8_t  hit[BLINK_WINDOW_SIZE];  /* 1=on-time 0=missed */
    uint32_t head;
    uint32_t count;
...
- ----------------------------------- */
  ...
  cfg->port_hop_interval  = 1;      /* Blink: hop every chunk (per-chunk port) */
  cfg->blink_interval_us  = 0;      /* Blink: 0 = auto-derive from RTT */
...
/* Blink-aware calibration: compute clock offset from RTT samples.
 * Called by run_transfer after mpcp_calibrate_collect_samples. */
static void blink_calibrate_from_rtt(mpcp_session_t      *sess,
                                      const mpcp_config_t *cfg,
                                      const double        *rtt_ms,
                                      uint32_t             n,
                                      double               ewma_rtt_ms)
{
    /* Without per-ping absolute timestamps we cannot compute a precise
     * clock_offset; set it to zero (RTT/2 in blink_arrival_time compensates). */
    (void)rtt_ms; (void)n;
    sess->clock_offset_ns = 0;

    /* Derive blink_interval from explicit config or auto from ewma_rtt */
    if (cfg->blink_interval_us > 0) {
        sess->blink_interval_ns = (uint64_t)cfg->blink_interval_us * 1000ULL;
    } else {
        /* Auto: half RTT + 3 ms guard margin */
        uint64_t rtt_half = (uint64_t)(ewma_rtt_ms * 500000.0); /* ms → ns/2 */
        sess->blink_interval_ns = rtt_half + 3000000ULL;
        /* Clamp: 5 ms ≤ interval ≤ 200 ms */
    ...
    /* Origin: now + 2 × interval gives receiver time to open its socket
     * before the first chunk's window opens. */
    sess->blink_origin_ns = mpcp_now_ns() + 2u * sess->blink_interval_ns;
...
static uint16_t blink_hop_port(const mpcp_session_t *sess,
                                ...
                                uint32_t              chunk_idx)
{
    /* Derive per-hop port: every cfg->port_hop_interval chunks we rotate.
     * hop_base = (chunk_idx / hop_interval) * hop_interval
     * port = HKDF(session_key, "blink-hop" || hop_base(4BE) || nonce) % range + base
     * When hop_interval==1 this degenerates to per-chunk ports (original behaviour).
     * When hop_interval==0 treat as 1. */
    ...
    uint8_t ikm[9 + 4 + 32]; /* "blink-hop"(9) + hop_base(4) + nonce(32) */
    ...
    if (mpcp_hkdf(sess->session_key, MPCP_SESSION_KEY_LEN,
        ...
        /* Deterministic fallback: XOR of keystream-derived port and hop_base */
        uint8_t ck[32]; uint16_t kp;
        mpcp_keystream_slot(sess->keystream, chunk_idx,
                            cfg->port_base, cfg->port_range, ck, &kp);
...
static uint64_t blink_arrival_time(const mpcp_session_t *sess,
                                    uint32_t              chunk_idx)
{
    /* T_arrival_i (receiver clock) =
     *   blink_origin_ns                    (sender t=0, in sender's clock)
     *   + chunk_idx * blink_interval_ns    (scheduled departure)
     *   - clock_offset_ns                  (convert sender→receiver clock)
     *   + blink_interval_ns / 2            (approx one-way flight ≈ RTT/2)
     *
     * clock_offset_ns = (receiver_ts - sender_ts) measured during calibration.
     * A positive offset means the receiver clock runs ahead of the sender. */
    if (sess->blink_interval_ns == 0) {
        /* Not yet calibrated — return current time (immediate) */
        return mpcp_now_ns();
    }
    uint64_t sched_sender = sess->blink_origin_ns
                            + (uint64_t)chunk_idx * sess->blink_interval_ns;
    /* Shift to receiver's clock domain and add half-interval as flight time */
    int64_t offset = sess->clock_offset_ns;
    uint64_t arrival_recv;
    if (offset >= 0) {
        arrival_recv = sched_sender + (uint64_t)offset
                       + sess->blink_interval_ns / 2u;
    } else {
        uint64_t abs_off = (uint64_t)(-offset);
        arrival_recv = (sched_sender > abs_off)
                       ? (sched_sender - abs_off + sess->blink_interval_ns / 2u)
                       : (sess->blink_interval_ns / 2u);
    }
    return arrival_recv;
}
...
/* ── blink_recv_window ──────────────────────────────────────────────────── */
/*
 * Sleep until (target_ns - 2ms), then poll fd for up to guard_ns (5ms).
 * Returns received byte count ≥ 0 on success, -1 on guard-band timeout.
 * The guard band keeps the socket "alive" for only ~5ms per chunk window,
 * dramatically reducing the fingerprint of an open listener socket.
 */
static ssize_t blink_recv_window(int fd, void *buf, size_t buf_len,
                                  uint64_t target_ns, uint64_t guard_ns,
                                  struct sockaddr_in *from_addr,
                                  socklen_t *from_len)
{
    const uint64_t PRE_SLEEP_NS = 2000000ULL; /* 2 ms lead */

    /* ── Phase 1: sleep until (target_ns - 2ms) ── */
    ...
    /* ── Phase 2: poll with strict guard_ns timeout ── */
    struct pollfd pfd = { .fd = fd, .events = POLLIN, .revents = 0 };
    /* Convert guard_ns to milliseconds for poll(), ceiling at 1ms */
    int guard_ms = (int)(guard_ns / 1000000ULL);
    if (guard_ms < 1) guard_ms = 1;

    int pret = poll(&pfd, 1, guard_ms);
    if (pret <= 0) {
        /* Timeout or error — missed window */
...
{
    /* Sleep until the scheduled transmit instant using CLOCK_MONOTONIC
     * absolute sleep — precise to the microsecond on real-time kernels. */
    uint64_t now = mpcp_now_ns();
...
static void blink_window_record(blink_window_t *w, bool on_time)
{
    ...
    /* Subtract out the oldest entry if full */
...
/* ── Clock-offset estimation during calibration ─────────────────────────── */
/*
 * Called after calibration to compute and store clock_offset_ns in sess.
 * Takes the array of (sender_send_ts, local_recv_ts, rtt_ns) triplets.
 *
 * clock_offset_ns = median(recv_ts_i - send_ts_i - rtt_i/2)
 *
 * A positive offset means the receiver clock is ahead of the sender.
 * This is used to convert sender-scheduled times into receiver-local times.
 */
                                        ...
                                        uint32_t          blink_interval_us)
{
    ...
    /* Compute per-sample offset and take median */
    int64_t *offsets = malloc(n * sizeof(int64_t));
    if (!offsets) { sess->clock_offset_ns = 0; return; }

    for (uint32_t i = 0; i < n; i++) {
        int64_t rtt_half_ns = (int64_t)(rtt_ms[i] * 500000.0); /* ms→ns/2 */
    ...
    /* Blink interval: use blink_interval_us if set, else derive from RTT */
    if (blink_interval_us > 0) {
        sess->blink_interval_ns = (uint64_t)blink_interval_us * 1000ULL;
    } else {
        /* Auto: interval = RTT/2 + 2ms guard + 1ms margin = ewma/2 + 3ms */
        uint64_t rtt_half = (uint64_t)(ewma_rtt_ms * 500000.0);
        sess->blink_interval_ns = rtt_half + 3000000ULL; /* +3ms */
        /* Clamp to reasonable range: 5ms – 200ms */
        if (sess->blink_interval_ns < 5000000ULL)
            sess->blink_interval_ns = 5000000ULL;
        if (sess->blink_interval_ns > 200000000ULL)
            sess->blink_interval_ns = 200000000ULL;
...
/* Build and send one chunk wire packet — Blink Engine extended header.
 * Wire layout (S18.3 + Blink):
 *   hdr(6) + seq(4) + retry(1) + flags(1) + plen(4) + nonce(24)
 *   + scheduled_time_ns(8) + ciphertext
 *   = MPCP_DATA_HDR_OFFSET(48) + ct_len
 *
 * scheduled_time_ns: sender's CLOCK_MONOTONIC time (ns, big-endian) at which
 * this chunk was intended to be transmitted. The receiver uses this together
 * with the calibrated clock_offset to compute its expected arrival time.
 */
...
memcpy(pkt + 16, slot->nonce, 24); /* nonce at offset 16, 24 bytes */

/* Blink: scheduled_time_ns at offset 40 (8 bytes, big-endian) */
uint64_t sched_ns = 0;
...
/* Ciphertext at offset 48 (MPCP_DATA_HDR_OFFSET) */
...
/* ── Blink Pacer: fixed-interval transmission ────────────────────────────
 * When blink_interval_ns > 0, pace each chunk transmit to its scheduled
 * absolute time using clock_nanosleep(CLOCK_MONOTONIC, TIMER_ABSTIME).
 * This keeps sender and receiver clocks aligned within RTT/2 accuracy. */
blink_pacer_t blink_pacer;
bool use_blink_pacer = (ctx->sess->blink_interval_ns > 0);
if (use_blink_pacer) {
    blink_pacer_init(&blink_pacer,
                     ctx->sess->blink_interval_ns,
                     ctx->sess->blink_origin_ns);
    fprintf(stderr, "[blink] sender pacer: interval=%.1fms origin_in=%.0fms\n",
            (double)ctx->sess->blink_interval_ns / 1e6,
            (double)(ctx->sess->blink_origin_ns - mpcp_now_ns()) / 1e6);
}
...
while (data_consumed < ctx->plan.n_chunks) { /* exit when all DATA chunks sent; ghost drain follows */
    ...
    /* Blink mode: use HKDF-derived hop port; else use keystream port */
    ...
    /* Blink pacing: sleep until the chunk's scheduled transmit instant */
    if (use_blink_pacer) blink_pacer_wait(&blink_pacer);
...
/* ──────────────────────────────────────────────────────────────────────────
 * Blink Receiver T1 — Temporal Stealth implementation
 *
 * Algorithm per chunk i:
 *   1. Compute T_arrival_i = blink_arrival_time(sess, i)
 *   2. Derive Blink hop port for this chunk
 *   3. Bind socket to hop port (close previous if hopping)
 *   4. Sleep until T_arrival_i - 2ms (pre-sleep phase)
 *   5. poll() for up to 5ms (guard band) on the bound socket
 *   6a. Packet received → process, send ACK, advance to next chunk
 *   6b. Guard expired (missed window) → record miss, advance to next chunk
 *
 * If Blink is not calibrated (blink_interval_ns == 0), falls back to the
 * original epoll-all-ports receive loop for backward compatibility.
 * ────────────────────────────────────────────────────────────────────────── */
...
/* ─────────────────────────────────────────────────────────────────────────
 * BLINK MODE: per-chunk temporal receive with clock_nanosleep + poll()
 * Each chunk has a scheduled arrival window; the socket is only open
 * during that ~5 ms window, dramatically reducing the traffic footprint.
 * ───────────────────────────────────────────────────────────────────────── */
if (sess->blink_interval_ns > 0) {
    ...
    /* Generous outer deadline — the blink windows handle per-chunk timing */
    uint64_t deadline_ns = mpcp_now_ns()
        + (uint64_t)cfg->chunk_retry_timeout * 4ULL * 1000000000ULL
        + 60000000000ULL;

    /* Single hopping socket — close/bind per hop interval */
    int      blink_fd  = -1;
    uint16_t cur_port  = 0;
    ...
    for (uint32_t i = 0; i < n && mpcp_now_ns() < deadline_ns; i++) {
        if (atomic_load_explicit(&ctx->abort_flag, memory_order_relaxed)) break;

        /* Ghost chunks occupy a time slot but need no socket */
        bool is_ghost = false;
        for (uint32_t g = 0; g < ctx->ghost_count; g++) {
            if (ctx->ghost_seqs[g] == i) { is_ghost = true; break; }
        }
        if (is_ghost) { got[i] = true; received++; continue; }

        /* ── Port Hopping: rebind only when the derived port changes ── */
        uint16_t needed_port = blink_hop_port(sess, cfg, i);
        if (needed_port != cur_port || blink_fd < 0) {
            if (blink_fd >= 0) { close(blink_fd); blink_fd = -1; }
            blink_fd = open_udp_socket_on_port(needed_port);
            if (blink_fd < 0) {
                fprintf(stderr, "[blink] bind port %u failed: %s\n",
                        needed_port, strerror(errno));
                atomic_fetch_add_explicit(&g_blink_missed_windows, 1,
                                         memory_order_relaxed);
                blink_window_record(&win, false);
                continue;
            }
            cur_port = needed_port;
        }

        /* ── Compute receiver-domain expected arrival time ── */
        uint64_t t_arrival = blink_arrival_time(sess, i);
        const uint64_t GUARD_NS = 5000000ULL;   /* 5 ms guard band */

        /* ── First attempt: sleep→poll within guard band ── */
        struct sockaddr_in from_addr;
        socklen_t          from_len = sizeof(from_addr);
        ssize_t nb = blink_recv_window(blink_fd, pkt, pkt_max,
                                        t_arrival, GUARD_NS,
                                        &from_addr, &from_len);

        if (nb < 0) {
            /* Missed primary window — allow one retry in the next slot */
            blink_window_record(&win, false);
            if (g_debug_stealth)
                fprintf(stderr, "  [blink] seq=%u missed primary (port=%u)\n",
                        i, cur_port);
            nb = blink_recv_window(blink_fd, pkt, pkt_max,
                                   t_arrival + GUARD_NS, GUARD_NS,
                                   &from_addr, &from_len);
            if (nb < 0) continue; /* retry also missed — advance */
        }
        ...
        /* Reject ghosts that slipped through */
        ...
        if (!mpcp_dedup_accept(&ctx->dedup, pkt_seq)) {
            blink_window_record(&win, true);
            continue;
        }

        /* ── Enqueue into pipeline ring ── */
        mpcp_ring_slot_t *slot = mpcp_ring_claim(&ctx->ring0);
        ...
        /* ── ACK ── */
        {
            ...
            sendto(blink_fd, &ack, sizeof(ack), 0,
        ...
        /* ── Progress bar ── */
        {
            ...
            int fill = (int)(40.0f * (float)(received + 1) /
                             (float)(ctx->plan.n_chunks > 0
                                     ? ctx->plan.n_chunks : 1));
            fprintf(stderr, "\r  %s[%s", C_GRAPE, C_RESET);
            for (int _b = 0; _b < 40; _b++) {
                if (_b < fill)
                    fprintf(stderr, "%s\xe2\x96\x88%s",
            ...
            fprintf(stderr,
                    "%s] %s%u/%u%s :%u miss:%.0f%%   ",
                    C_GRAPE, C_PLUM, received + 1, ctx->plan.n_chunks,
                    C_RESET, cur_port,
                    blink_window_miss_rate(&win) * 100.0);
#else
            fprintf(stderr, "\r  [%u/%u] :%u miss=%.0f%%   ",
                    received + 1, ctx->plan.n_chunks, cur_port,
                    blink_window_miss_rate(&win) * 100.0);
#endif
            fflush(stderr);
        }
        ...
        got[i]  = true;
        received++;
    } /* end per-chunk Blink loop */

    if (blink_fd >= 0) close(blink_fd);

    /* Print session Blink stats */
    if (g_debug_stealth) {
        uint64_t tot_miss = atomic_load_explicit(&g_blink_missed_windows,
                                                 memory_order_relaxed);
        uint64_t tot_hit  = atomic_load_explicit(&g_blink_on_time_windows,
                                                 memory_order_relaxed);
        fprintf(stderr,
                "\n  [blink] session total: %lu on-time  %lu missed"
                "  (%.1f%% hit)\n",
                (unsigned long)tot_hit, (unsigned long)tot_miss,
                (tot_hit + tot_miss) > 0
                    ? 100.0 * (double)tot_hit / (double)(tot_hit + tot_miss)
                    : 100.0);
    }
    free(got);
    ...
    /* ─────────────────────────────────────────────────────────────────────
     * LEGACY FALLBACK: bind all ports simultaneously, poll with MSG_DONTWAIT.
     * Used when Blink calibration has not run (blink_interval_ns == 0).
     * ───────────────────────────────────────────────────────────────────── */
...
static void test_blink_engine(void)
{
    ...
    /* ── T-BLINK-01: blink_pacer pacing accuracy ── */
    {
        const uint64_t INTERVAL_NS = 10000000ULL; /* 10 ms */
        blink_pacer_t  pacer;
        uint64_t       origin = mpcp_now_ns() + 5000000ULL; /* +5ms */
        blink_pacer_init(&pacer, INTERVAL_NS, origin);

        /* Advance two slots and measure elapsed time */
        uint64_t t0 = mpcp_now_ns();
        blink_pacer_wait(&pacer); /* slot 0: wake ~origin */
        blink_pacer_wait(&pacer); /* slot 1: wake ~origin + interval */
        uint64_t elapsed = mpcp_now_ns() - t0;

        /* Expected: ~15ms (5ms until origin + 10ms slot) ± 2ms tolerance */
    ...
    /* ── T-BLINK-02: blink_window sliding stats ── */
    {
        blink_window_t w;
        blink_window_init(&w);
        ...
        /* Miss rate = 5/15 = 0.333... */
        P3_CHECK("T-BLINK-02a window miss rate correct",
                 miss > 0.32 && miss < 0.35);

        /* Fill past BLINK_WINDOW_SIZE to test sliding */
    ...
    /* ── T-BLINK-03: blink_hop_port determinism ── */
    {
        mpcp_session_t sess;
        memset(&sess, 0, sizeof(sess));
        randombytes_buf(sess.session_nonce, MPCP_SESSION_NONCE_LEN);
        ...
        mpcp_config_t cfg;
        mpcp_config_defaults(&cfg);
        cfg.port_hop_interval = 1; /* per-chunk */

        /* Two independent calls must return identical port */
        ...
        /* Different chunks must generally differ */
        uint16_t p3 = blink_hop_port(&sess, &cfg, 6);
        /* Note: collision possible but extremely rare */
        bool diff = (p1 != p3);
        P3_CHECK("T-BLINK-03b hop_port varies across chunks", diff);

        /* Verify port in configured range */
        ...
        /* With hop_interval=4, chunks 0-3 should share the same port */
    ...
    /* ── T-BLINK-04: blink_arrival_time formula ── */
    {
        mpcp_session_t sess;
        memset(&sess, 0, sizeof(sess));
        sess.blink_interval_ns = 10000000ULL; /* 10 ms */
        sess.blink_origin_ns   = 1000000000ULL; /* t=1s */
        sess.clock_offset_ns   = 0;

        uint64_t t0 = blink_arrival_time(&sess, 0);
        uint64_t t5 = blink_arrival_time(&sess, 5);

        /* chunk 0 arrival = origin + 0*interval - 0 + interval/2
         *                 = 1000ms + 5ms = 1005ms */
        bool t0_ok = (t0 == 1005000000ULL);
        P3_CHECK("T-BLINK-04a arrival_time seq=0 correct", t0_ok);

        /* chunk 5 = origin + 5*10ms + 5ms = 1000+50+5 = 1055ms */
        bool t5_ok = (t5 == 1055000000ULL);
        P3_CHECK("T-BLINK-04b arrival_time seq=5 correct", t5_ok);

        /* Positive clock offset: receiver is ahead, arrival shifts earlier */
        sess.clock_offset_ns = 2000000LL; /* +2ms */
    ...
    /* ── T-BLINK-05: blink_recv_window guard band timing ── */
    {
        /* Create a local socket pair and test that blink_recv_window
         * correctly handles a timeout (no sender) — guard band expires. */
        ...
        if (server_fd >= 0) {
            ...
            /* Target 2ms from now with a 5ms guard — should time out cleanly */
            uint8_t buf[64];
            struct sockaddr_in from;
            ...
            /* Must have waited at least 5ms (guard) + ~2ms pre-sleep */
            P3_CHECK("T-BLINK-05b guard band minimum duration >=5ms",
                     elapsed_ms >= 4.5);
            /* Must not have waited more than 15ms (sanity) */
    ...
    /* ── T-BLINK-06: scheduled_time embedded in chunk wire format ── */
    {
        /* Build a fake ring slot and call send_chunk_packet into a buffer.
         * Verify that bytes [40..47] of the wire contain the expected
         * scheduled_time (big-endian uint64). */
        mpcp_session_t sess;
        memset(&sess, 0, sizeof(sess));
        sess.blink_interval_ns = 10000000ULL;
        sess.blink_origin_ns   = 500000000ULL;
        /* No real socket needed — use a loopback sendto and capture below */

        /* Just verify the mathematical relationship without network I/O */
        uint64_t expected_sched =
            sess.blink_origin_ns + 3ULL * sess.blink_interval_ns;
        /* expected_sched = 500ms + 3*10ms = 530ms */
        bool formula_ok = (expected_sched == 530000000ULL);
        P3_CHECK("T-BLINK-06a scheduled_time formula: origin + seq * interval",
                 formula_ok);
        /* Verify big-endian encoding round-trip */
    ...
    /* ── T-BLINK-07: blink_compute_clock_offset median calculation ── */
    {
        mpcp_session_t sess;
        memset(&sess, 0, sizeof(sess));
        /* Simulated offset samples: receiver sees packets 3ms late on average */
        /* offset_i = recv_ts_i - send_ts_i - rtt_i/2 */
        /* Use blink_calibrate_from_rtt with known ewma */
        mpcp_config_t cfg;
        mpcp_config_defaults(&cfg);
        cfg.blink_interval_us = 0; /* auto from RTT */

        double fake_rtts[5] = { 10.0, 11.0, 10.5, 9.5, 10.0 }; /* ms */
        double ewma = 10.2; /* ms */
        blink_calibrate_from_rtt(&sess, &cfg, fake_rtts, 5, ewma);

        /* blink_interval = ewma/2 + 3ms = 5.1 + 3 = 8.1ms, clamped ≥5ms */
        bool interval_ok = (sess.blink_interval_ns >= 5000000ULL &&
                            sess.blink_interval_ns <= 200000000ULL);
        P3_CHECK("T-BLINK-07a calibrated interval within valid range",
                 interval_ok);

        /* With explicit interval override */
        cfg.blink_interval_us = 25000; /* 25ms */
        blink_calibrate_from_rtt(&sess, &cfg, fake_rtts, 5, ewma);
        bool override_ok = (sess.blink_interval_ns == 25000000ULL);
        P3_CHECK("T-BLINK-07b explicit blink_interval_us respected", override_ok);

        /* Origin should be in the future */
...
/* Blink-extended transfer_info_send.
 * Wire format: nonce(24) || ciphertext(37) = 61 bytes
 * Plaintext (21 bytes):
 *   n_chunks(4 BE) | flags(1) | blink_origin_ns(8 BE) | blink_interval_ns(8 BE)
 * CT = 21 + 16 (Poly1305 tag) = 37 bytes
 */
static int transfer_info_send(const mpcp_session_t *sess,
                               ...
                               uint8_t  flags)
{
    /* Plaintext: n_chunks(4) | flags(1) | blink_origin(8) | blink_interval(8) */
    uint8_t plain[21];
    ...
    /* Blink origin (8 bytes, big-endian) */
    uint64_t bo_be = __builtin_bswap64(sess->blink_origin_ns);
    memcpy(plain + 5, &bo_be, 8);

    /* Blink interval (8 bytes, big-endian) */
...
/* Blink-extended transfer_info_recv.
 * Accepts 61-byte wire format (Blink): nonce(24) || CT(37)
 * Also accepts legacy 45-byte format for backward compat.
 * Decodes blink_origin_ns and blink_interval_ns into *sess. */
                               ...
                               uint8_t  *flags_out)
{
    ...
    /* Buffer large enough for both 45-byte and 61-byte formats */
    uint8_t wire[64];
    ...
    if (n == 61) {
        /* Blink format: nonce(24) || CT(37) → plain(21) */
        ...
        *flags_out = plain[4];
        ...
        /* Decode Blink parameters */
        uint64_t bo_be, bi_be;
        memcpy(&bo_be, plain + 5,  8);
        memcpy(&bi_be, plain + 13, 8);
        sess->blink_origin_ns   = __builtin_bswap64(bo_be);
        sess->blink_interval_ns = __builtin_bswap64(bi_be);

        fprintf(stderr, "[blink] receiver: interval=%.1fms  origin_in=%.0fms\n",
                (double)sess->blink_interval_ns / 1e6,
                (double)(sess->blink_origin_ns - mpcp_now_ns()) / 1e6);

    } else if (n == 45) {
        /* Legacy format: nonce(24) || CT(21) → plain(5) */
        memcpy(nonce, wire, 24);
        uint8_t plain5[5];
        ...
        *flags_out = plain5[4];
        /* No Blink params — receiver uses legacy mode */
...
static int run_transfer(void)
{
    ...
    if (is_sender) {
        ...
        /* Blink Engine: compute clock offset and blink interval from RTT */
        blink_calibrate_from_rtt(&sess, &cfg, rtt_samples, rtt_count,
                                  rtt.ewma_rtt);

        /* Blink Engine: compute clock_offset and blink_interval from RTT data.
         * Must be called before key exchange so origin time is available
         * when transfer_info_send transmits it to the receiver. */
        blink_calibrate_from_rtt(&sess, &cfg, rtt_samples, rtt_count,
                                 rtt.ewma_rtt);
...
{
    /* --debug-stealth: enable Blink Engine missed-window diagnostics */
    ...
    /* Blink Engine: --debug-stealth flag enables per-window timing output */
    if (argc >= 2 && strcmp(argv[1], "--debug-stealth") == 0) {
        g_debug_stealth = true;
        /* Shift argv so interactive mode runs normally after flag is consumed */
        /* Fall through to interactive mode with debug enabled */
